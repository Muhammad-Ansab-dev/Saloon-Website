'use client';
// ContactPage — Full-page contact view composed of four cards in a 2×2 grid:
// (1) Zurich branch card (image left, all branch info right), (2) Paris branch
// card (same layout), (3) booking form (the shared BookingForm — same widget
// used by the booking modal, backed by live service/stylist data), (4)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          {LOCATIONS.map((loc, idx) => (
            <motion.div
              ...
              className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 border border-neutral-300 bg-white overflow-hidden"
            >
          ...
          <motion.div
            ...
            className="border border-neutral-300 bg-white p-7 sm:p-9"
          >
            <div className="flex items-center gap-2">
              <Scissors className="w-4 h-4 text-black" />
              <h2 className="font-editorial text-lg sm:text-xl font-black uppercase tracking-[0.15em] text-black border-b border-black/20 pb-2 inline-block">
                Book an Appointment
              </h2>
            </div>
            ...
            <div className="mt-7">
              <BookingForm
                key={bookingKey}
                onDone={() => setBookingKey((prevKey) => prevKey + 1)}
              />
            </div>
          </motion.div>
